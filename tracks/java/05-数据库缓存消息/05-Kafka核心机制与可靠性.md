...
> **一句话定位**：Kafka 以分区追加日志实现高吞吐和顺序读写，副本机制提供容错，消费组让同一分区在组内由一个消费者处理。
...
> **图：Kafka 任务、线程与实例的并行关系**
> （图源与许可见 [assets/CREDITS.md](./assets/CREDITS.md)）
>
> 对照本文阅读时，把图中的输入、状态、执行器和输出依次映射为：
> **Producer 按 key 选择分区 → Leader 追加顺序日志 → Follower 拉取复制 → Consumer Group 提交 offset 并处理重平衡**。
...
- **入口与约束**：`Producer 按 key 选择分区` 决定后续处理的语义、权限和容量边界。
- **核心决策**：`Leader 追加顺序日志` 与 `Follower 拉取复制` 是最容易答成“只会背名词”的地方，要讲清状态如何变化。
- **执行与副作用**：中间步骤必须说明失败是否可重试、是否会重复，以及资源何时释放。
- **完成条件**：`Consumer Group 提交 offset 并处理重平衡` 不能只看“没有抛异常”，还要有业务结果、指标或持久化证据。
...
| 状态 | 需要回答的问题 |
|---|---|
| 输入 | `Producer 按 key 选择分区` 是否已经校验语义、权限、大小与版本？ |
| 中间状态 | `Follower 拉取复制` 能否持久化、观测或在并发下保持不变量？ |
| 副作用 | 失败重试会不会重复执行？是否有幂等键、事务或补偿？ |
| 完成 | `Consumer Group 提交 offset 并处理重平衡` 由什么证据确认？调用方能否区分成功、部分成功与失败？ |
...
2. **再沿关键路径定位**：从 `Producer 按 key 选择分区` 逐步检查到 `Consumer Group 提交 offset 并处理重平衡`，不要跳过排队、缓存、代理或外部依赖。
3. **区分原因和结果**：高 CPU、线程多、token 多、连接满可能只是症状；用日志、指标、Trace 和状态快照互相印证。
4. **最后验证修复**：用最小复现、压力或故障注入证明问题消失，同时保留一键回滚。

最常见的错误是：**消费者处理超过 max.poll.interval 导致频繁 rebalance。**
...
核心状态：Follower 拉取复制
完成证据：Consumer Group 提交 offset 并处理重平衡
高频坑：消费者处理超过 max.poll.interval 导致频繁 rebalance。
```
...
1. **请用一分钟说明Kafka 核心机制与可靠性的核心目标与工作机制。**
